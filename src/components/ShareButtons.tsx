...
import { Share2, Copy, Check, MapPin, Users } from 'lucide-react';
import { shareUtils, getShareableUrl, getShareText } from '../utils/share';
...
export const ShareButtons: React.FC = () => {
  ...
  const handleWhatsAppShare = () => {
    ...
    shareUtils.whatsapp(url, text);
  };

  const handleFacebookShare = () => {
    const url = getShareableUrl();
    shareUtils.facebook(url);
  };
    ...
    <div className="mt-6 pt-6 border-t border-gray-200">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-gray-600 text-white rounded-lg font-medium hover:bg-gray-700 transition-colors text-sm"
        >
          {copied ? (
            <>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-teal-500 text-white rounded-lg font-medium hover:bg-teal-600 transition-colors text-sm"
        >
          <Users className="w-4 h-4" />
          Mantenha-se informado pelo nosso grupo de WhatsApp
        </button>
      </div>
    </div>
  );
